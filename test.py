...
def BatchTest(FilePath,MaskBinFile,MaskOriFile,MaskJsonFile,MaskTxtFile):
    ...
    for index, img_file in enumerate(img_test):
        ...
        pred_resized = cv2.resize(pred, (s_w, s_h), interpolation=cv2.INTER_NEAREST)
        pred_resized = pred_resized * 255

        s_img_masked = s_img.copy()
        s_img_masked[:, :, 0] = s_img_masked[:, :, 0] * (pred_resized / 255)
        s_img_masked[:, :, 1] = s_img_masked[:, :, 1] * (pred_resized / 255)
        s_img_masked[:, :, 2] = s_img_masked[:, :, 2] * (pred_resized / 255)

        # Reverse generate pred_resized_new
        pred_resized_new = np.zeros((s_h, s_w), dtype='uint8')
        non_black_pixels = np.any(s_img_masked != 0, axis=-1)
        pred_resized_new[non_black_pixels] = 255
        ...
        print(img_name)
        savepath = os.path.join(MaskBinFile, img_name)
        savepathOri = os.path.join(MaskOriFile, img_name)
        savepathNewPred = os.path.join(MaskBinFile, os.path.splitext(img_name)[0] + '_new.png')
        label_path = os.path.join(MaskJsonFile, os.path.splitext(img_name)[0] + '.json')
        txt_path = os.path.join(MaskTxtFile, os.path.splitext(img_name)[0] + '.txt')

        # 保存标注结果
        save_annotation(pred_resized, MaskBinFile, img_name, label_path, txt_path)
        cv2.imwrite(savepath, pred_resized_new)
        cv2.imwrite(savepathOri, s_img_masked)
        # cv2.imwrite(savepathNewPred, pred_resized_new)
